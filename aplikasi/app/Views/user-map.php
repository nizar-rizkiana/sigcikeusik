...
<html lang="en">
  ...
  <body>
    ...
    L.marker([<?= $sek['koord_x'] ?>, <?= $sek['koord_y'] ?>], {icon: sd}).addTo(map).bindPopup('<?= ($sek['gambar'] != null) ? '<img src="'.base_url().'/gambar/'.$sek['gambar'].'" width="100%"></img>' : '' ?>'
    +'<p class="text-center fw-bold p-0 m-0"><?= $sek['nama_sekolah'] ?></p>'
    <?php endif; ?>
    <?php  if($sek['jenjang'] == 'SMP Sederajat') : ?>
    L.marker([<?= $sek['koord_x'] ?>, <?= $sek['koord_y'] ?>], {icon: sltp}).addTo(map).bindPopup('<?= ($sek['gambar'] != null) ? '<img src="'.base_url().'/gambar/'.$sek['gambar'].'" width="100%"></img>' : '' ?>'
    +'<p class="text-center fw-bold p-0 m-0"><?= $sek['nama_sekolah'] ?></p>'
    <?php endif; ?>
    <?php  if($sek['jenjang'] == 'SMA Sederajat') : ?>
    L.marker([<?= $sek['koord_x'] ?>, <?= $sek['koord_y'] ?>]).addTo(map).bindPopup('<?= ($sek['gambar'] != null) ? '<img src="'.base_url().'/gambar/'.$sek['gambar'].'" width="100%"></img>' : '' ?>'
    +'<p class="text-center fw-bold p-0 m-0"><?= $sek['nama_sekolah'] ?></p>'
    <?php endif; ?>
